...
// The code is adopted from SNHU CS-330 tutorial,
// as well as Learn OpemGL & Github websites
// The shapes, textures, and object are dawned based on
// a real picture as submitted in the beginning of the course


#include "BuildShapes.h"	// contains function decelerations for the majority of the shapes used in this program (Pyramids, Cones, Cylinder, etc.)
#include "Meshes.h"			// contains function deceleration for mesh
#include "BuildTheScene.h"	// contains function decelerations for getting the main parameters of the shapes to be built and their location
#include "camera.h"			// contains function decelerations for the camera movements
#include "info.h"			// contains function deceleration for printing control keys on the console
...
// window title
const char* const WINDOW_TITLE = "Final Project CS 330 *** Mahdi Bigdely";

// window width, height
const int WINDOW_WIDTH = 1200;
const int WINDOW_HEIGHT = 800;

GLightMesh fillLighMesh;
...
// boolean variable to switch between perspective and ortho views
bool perspective = false;



// camera variable deceleration
Camera gCamera(glm::vec3(0.0f, 10.0f, 18.0f), glm::vec3(0.0f, 1.0f, 0.0f), -90.0f, -25.0f);
...
// used for find the timing between each frame and handle the rendering
float gDeltaTime = 0.0f; // time between current frame and last frame
float gLastFrame = 0.0f;


// spotlight color and position and scale
glm::vec3 gSpotLightColor(1.0f, 1.0f, 1.0f);
glm::vec3 gSpotLightPosition(-3.0f, 1.5f, -2.6f);
glm::vec3 gSpotLightScale(0.1f);

// key-light color and position and scale
glm::vec3 gKeyLightColor(1.0f, 1.0f, 1.0f);
glm::vec3 gKeyLightPosition(-2.5f, 4.0f, -1.5f);
glm::vec3 gKeyLightScale(0.12f);

bool gSpotLightRotate = true;

// initialize program
bool UInitialize(int, char* [], GLFWwindow** window);
// sets window size
void UResizeWindow(GLFWwindow* window, int width, int height);
// receives and take action on user input
...
bool UCreateShaderProgram(const char* vtxShaderSource, const char* fragShaderSource,
	GLuint& programId);
// frees up the memory when program ends/terminates
...
void main()
{
	...
	float spotStrength = 0.10f; // sets fill light a strength of 10% per assignment
	float keyStrength = 0.6f; // sets key light a strength of 100% per assignment
	vec3 spot = spotStrength * lightColor; // generates fill light color (white light with 10% strength)
	vec3 key = keyStrength * keyLightColor; // generates key light color (green light with 100% strength)

	//Calculate Diffuse lighting*/
	vec3 norm = normalize(vertexNormal); // normalizes vectors to 1 unit
	vec3 lightDirection = normalize(lightPos - vertexFragmentPos); // calculates distance (light direction) between light source and fragments/pixels on pyramids
	vec3 keyLightDirection = normalize(keyLightPos - vertexFragmentPos); // calculate distances (light direction) between light source and fragments/pixels on pyramidspyramids
...
void main()
{
	gl_Position = projection * view * model * vec4(position, 1.0f); // transforms vertices's into clip coordinates
}
);
...
void main()
{
	fragmentColor = vec4(1.0f); // Sets color to white (1.0f,1.0f,1.0f) with alpha 1.0
}
);
...
int main(int argc, char* argv[])
{
	...
	// Create Light Object
	UCreateLightMesh(fillLighMesh);
	UCreateLightMesh(keyLightMesh);

	// connects the texture to the correct sampler  
	glUseProgram(gShaderProgram);
	// We set the texture as texture unit 0
	glUniform1i(glGetUniformLocation(gShaderProgram, "uTexture"), 0);

	// boolean variable to make sure the info for user in only printed once on the console, 
	// and at the beginning of the program running
	bool infoPrinted = false;


	// rendering loop
	// checks if window is closed/terminated 
	while (!glfwWindowShouldClose(gWindow))
	{
		...
		// print the control keys on the console, only once and when program loads for the 1st time
		if (!infoPrinted)
		{
			// prints the control keys info on the console when the program loads for the first time
			info::printUserGuide();
			// prevents reprinting key controls info on the console on each frame
			infoPrinted = true;
		}
...
void UProcessInput(GLFWwindow* window)
{
	...
	// exits program if escape or F4 is pressed
	...
	// the polygon are drawn filled
	if (glfwGetKey(window, GLFW_KEY_LEFT) == GLFW_PRESS)
		glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);


	// uses W S A D keys to move camera around
	...
	// uses E Q keys to move camera upward/fownward
	...
	// I could use perspective =!perspective to switch the view
	// using only P key press, however using two different keys (I, and P)
	// to change the view was smoother and more responsive
	if (glfwGetKey(window, GLFW_KEY_P) == GLFW_PRESS)
		perspective = true;
	if (glfwGetKey(window, GLFW_KEY_O) == GLFW_PRESS)
		perspective = false;

	// alternative solution which was not as smooth as using only one key "P"
	/*if (glfwGetKey(window, GLFW_KEY_P) == GLFW_PRESS)
		perspective =!perspective
	*/

	// pause and resume spot light to rotate around the scene
	if (glfwGetKey(window, GLFW_KEY_LEFT_SHIFT) == GLFW_PRESS && !gSpotLightRotate)
		gSpotLightRotate = true;
	else if (glfwGetKey(window, GLFW_KEY_RIGHT_SHIFT) == GLFW_PRESS && gSpotLightRotate)
		gSpotLightRotate = false;

	// another solution to change camera's speed
	// in addition to the mouse scrolls, B / N keys can change camera's speed
	if (glfwGetKey(window, GLFW_KEY_B) == GLFW_PRESS)
	{
		if (gCamera.MovementSpeed < 10.0f)
		{
			gCamera.MovementSpeed += 0.0011f; // used small value to make camera's speed changes smoother
		}
		else
...
// handles resizing window events
void UResizeWindow(GLFWwindow* window, int width, int height)
{
...
void UMouseScrollCallback(GLFWwindow* window, double xoffset, double yoffset)
{
	// changes camera speed by mouse scroll (can use B /N keys as well)
...
void UMouseButtonCallback(GLFWwindow* window, int button, int action, int mods)
{
	...
	case GLFW_MOUSE_BUTTON_LEFT:
	{
		...
		{
			cout << "Left Mouse Button was Pressed => Camera position is reset." << endl;
	...
	default:
		cout << "Unhanded mouse button event" << endl;
		break;
	}
}
...
void URender(vector<GLMesh> scene)
{
	// rotates/circles the fill light around the scene
	constexpr float angularVelocity = glm::radians(43.0f);
	if (gSpotLightRotate)
	{
		glm::vec4 newPosition = glm::rotate(angularVelocity * gDeltaTime, glm::vec3(0.0f, 1.2f, 0.0f)) * glm::vec4(gSpotLightPosition, 1.0f);
	...
	glBindVertexArray(fillLighMesh.vao);
	...
	// draws the Spotlight mesh
	glDrawArrays(GL_TRIANGLES, 0, fillLighMesh.nVertices);
